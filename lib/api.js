...
class evok extends client {
    ...
    digitalOutputs() {
        return this.devices().filter(device => device.dev === 'relay' && device.relay_type === 'digital').sort(this.sort)
    }

    digitalOutput(circuit, state) {
        let output = this.digitalOutputs().find(device => device.circuit === circuit)
    ...
    leds() {
        return this.devices().filter(device => device.dev === 'led').sort(this.sort)
    }
    ...
    analogueInputs() {
        return this.devices().filter(device => device.dev === 'ai').sort(this.sort)
    }
    ...
    analogueOutputs() {
        return this.devices().filter(device => device.dev === 'ao').sort(this.sort)
    }
